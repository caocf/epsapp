...
import lib.pulltorefresh.internal.FlipLoadingLayout;
import lib.pulltorefresh.internal.LoadingLayout;
import lib.pulltorefresh.internal.RotateLoadingLayout;
import lib.pulltorefresh.internal.Utils;
import lib.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    static final float FRICTION = 3.0f; // 摩擦力：值越大，滑动反应越迟钝（默认2.0）

    public static final int SMOOTH_SCROLL_DURATION_MS = 200; // 上、下拉松手后，平滑归位的持续时间
    ...
    // add by poet
    protected void onSetLoadingTextColor(int color) {

    }

    protected void onSetLoadingTextStart(State state, String text) {

    }

    protected void onSetLoadingTextEnd(State state, String text) {
    ...
    public void setLoadingTextColor(int color) {
        ...
        onSetLoadingTextColor(color);
    }

    public void setLoadingTextStart(State state, String text) {
        switch (state) {
        case PULL_TO_REFRESH:
            mHeaderLayout.setPullLabel(text);
            break;
        case RELEASE_TO_REFRESH:
            mHeaderLayout.setReleaseLabel(text);
            break;
        case REFRESHING:
            mHeaderLayout.setRefreshingLabel(text);
            break;
        }
        onSetLoadingTextStart(state, text);
    }

    public void setLoadingTextEnd(State state, String text) {
        switch (state) {
        case PULL_TO_REFRESH:
            mFooterLayout.setPullLabel(text);
            break;
        case RELEASE_TO_REFRESH:
            mFooterLayout.setReleaseLabel(text);
            break;
        case REFRESHING:
            mFooterLayout.setRefreshingLabel(text);
            break;
        }
        onSetLoadingTextEnd(state, text);
    }
